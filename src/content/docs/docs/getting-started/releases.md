...
* `new` #4607 : Add bit size information by @ildyria.
  > Small debugging info on the diagnostic page which will help us figuring out if the users are running a 32 or 64-bit version of php.
...
* `new` #4613 : Keep image selection and position when many images are selected by @jphuguet.
  > When selecting many images and doing an action, once completed the selection was lost.
...
* `fix` #4592 : Fix slippery zip extraction by @ildyria.
...
* `fix` #4620 : Just print the message and die by @ildyria.
  > In the rare case where the user has a broken configuration, it was possible that Lychee crashes before the logging components
